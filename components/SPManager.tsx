...
import { LayoutGrid, Plus, Clock, AlertCircle, CheckCircle2, TrendingUp, Calendar, Search, Check, History, X, ArrowRight } from 'lucide-react';
...
const SPManager: React.FC = () => {
    ...
    const getCompletionTime = (bill: Bill) => {
        if (bill.spHistory && bill.spHistory.length > 0) {
            return new Date(bill.spHistory[bill.spHistory.length - 1].date).getTime();
        }
        return new Date(bill.date).getTime();
    };

    const pendingBillsList = useMemo(() => {
        const pending = searchFilteredBills.filter(b => {
    ...
    const totalUpdatedSP = useMemo(() => {
        return searchFilteredBills.reduce((sum, b) => sum + (b.spUpdatedAmount || 0), 0);
    }, [searchFilteredBills]);
    ...
    const handleFilterSelect = (type: 'Select' | 'Today' | 'This Week' | 'This Month') => {
        ...
        setSelectedBillIds(new Set()); // Clear selection when filter changes
    ...
    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            const visibleIds = activeTab === 'pending'
                ? pendingBillsList.filter(b =>
                    b.customerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    b.id.toLowerCase().includes(searchTerm.toLowerCase())
                ).map(b => b.id)
                : completedBillsList.filter(b =>
                    b.customerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    b.id.toLowerCase().includes(searchTerm.toLowerCase())
                ).map(b => b.id);
            setSelectedBillIds(new Set(visibleIds));
        } else {
            ...
            await Promise.all(Array.from(selectedBillIds).map(async (id) => {
                ...
                if (bill) {
                    ...
                    if (amountAdded > 0) {
                        ...
                        // Send WhatsApp notification for SP done
                        ...
                        if (customer && customer.phone) {
                            ...
                            import('../services/smsService').then(({ sendSPDoneWhatsapp }) => {
                                sendSPDoneWhatsapp(
                                    customer.phone,
                                    bill.id,
                                    bill.totalSp,
                                    ...
                                    if (success) {
                                        console.log(`SP Done WhatsApp sent for bill ${bill.id}`);
                                    }
                                }).catch(err => {
                                    console.error(`Failed to send SP Done WhatsApp for bill ${bill.id}:`, err);
                                });
                            });
                        }
                    }
    ...
    return (
        <div className="flex flex-col h-full bg-[#F3F4F6] text-gray-800 font-sans overflow-hidden">
            <main className="flex-1 overflow-hidden flex flex-col max-w-7xl mx-auto w-full px-3 sm:px-6 lg:px-8 pt-2 pb-0 md:py-6">
                ...
                {/* Stats & Filters Row */}
                <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 md:gap-4 mb-3 md:mb-6 shrink-0">
                    {/* Stats Bar */}
                ...
                <div className="flex flex-col flex-1 min-h-0 bg-transparent md:bg-white md:rounded-xl md:shadow-sm md:border md:border-gray-200 overflow-hidden">
                    ...
                    <div className="flex md:hidden w-full border-b border-gray-200 mb-4 px-1">
                        <button
                            ...
                            className={`flex-1 justify-center pb-3 text-sm font-semibold transition-all flex items-center gap-2 ${activeTab === 'pending'
                                ? 'text-gray-900 border-b-2 border-[#00d084]'
                                : 'text-gray-500 border-b-2 border-transparent'
                                }`}
                        >
                            Pending
                            {pendingCount > 0 && (
                                <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${activeTab === 'pending' ? 'bg-[#00d084]/20 text-[#00d084]' : 'bg-gray-100 text-gray-500'}`}>
                                    {pendingCount}
                                </span>
                            )}
                        </button>
                        ...
                        >
                            Completed
                            {completedCount > 0 && (
                                <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${activeTab === 'completed' ? 'bg-[#00d084]/20 text-[#00d084]' : 'bg-gray-100 text-gray-500'}`}>
                                    {completedCount}
                                </span>
                            )}
                        </button>
                    ...
                    <div className="flex-none px-1 md:px-5 mb-4 md:mb-0 bg-transparent md:bg-gray-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
                        ...
                        <div className="hidden md:flex bg-gray-200 p-1 rounded-lg w-auto">
                            ...
                            <button
                                onClick={() => setActiveTab('completed')}
                                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'completed'
                                    ? 'bg-white text-gray-900 shadow-sm'
                                    : 'text-gray-500 hover:text-gray-700'
                                    }`}
                            >
                        ...
                        <div className="relative w-full md:w-96 group">
                            ...
                            <input
                                ...
                                className="block w-full pl-11 pr-4 py-3 md:py-2 bg-white md:bg-white border md:border border-transparent md:border-gray-200 rounded-full md:rounded-lg leading-5 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#00d084]/50 shadow-sm md:shadow-none sm:text-sm transition-all"
                    ...
                    <div className="hidden md:grid grid-cols-12 gap-4 px-6 py-3 bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-500 uppercase tracking-wider flex-none items-center">
                        <div className="col-span-1 flex justify-center">
                            <input
                                type="checkbox"
                                className="w-4 h-4 rounded border-gray-300 text-[#141e26] focus:ring-[#141e26] cursor-pointer accent-[#141e26]"
                                onChange={(e) => handleSelectAll(e.target.checked)}
                                checked={
                                    activeTab === 'pending'
                                        ? pendingBillsList.length > 0 && selectedBillIds.size === pendingBillsList.filter(b =>
                                            b.customerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                                            b.id.toLowerCase().includes(searchTerm.toLowerCase())
                                        ).length
                                        : completedBillsList.length > 0 && selectedBillIds.size === completedBillsList.filter(b =>
                                            b.customerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                                            b.id.toLowerCase().includes(searchTerm.toLowerCase())
                                        ).length
                                }
                    ...
                    <div className="overflow-y-auto flex-1 px-0 md:px-0 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-gray-300 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-gray-400">
                        {activeTab === 'pending' ? (
                            <div key="pending-container" className="md:divide-y md:divide-gray-200 pb-28 md:pb-0">
                                <SPPending
                                    bills={pendingBillsList}
                                    searchTerm={searchTerm}
                                    onUpdate={() => { setBills(store.getBills()); setSelectedBillIds(new Set()); }}
