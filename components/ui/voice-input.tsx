...
export function VoiceInput({
    ...
    setListening: externalSetListening,
}: React.ComponentProps<"div"> & VoiceInputProps) {
    const [_internalListening, _setInternalListening] = React.useState<boolean>(false)
    const [_time, _setTime] = React.useState<number>(0)

    // Use external state if provided, otherwise use internal
    const _listening = externalListening !== undefined ? externalListening : _internalListening
    const _setListening = externalSetListening !== undefined ? externalSetListening : _setInternalListening
    ...
    return (
        <div className={cn("flex flex-col items-center justify-center", className)}>
            <motion.div
                className="flex p-2 border items-center justify-center rounded-full cursor-pointer bg-white hover:bg-gray-50 transition-colors"
                layout
                transition={{
                    layout: {
                        duration: 0.4,
